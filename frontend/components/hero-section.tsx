import { STRAPI_BASE_URL } from "@/services/strapi";
...
export function HeroSection({ data }: { readonly data: {
  ...
  // Strapi returns a single image object here, so access its properties directly.
  const image = data.image[0];
  const imageURL =
    image.url?.startsWith("http") ? image.url : `${STRAPI_BASE_URL}${image.url}`;
